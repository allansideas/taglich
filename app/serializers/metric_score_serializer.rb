class MetricScoreSerializer < ActiveModel::Serializer
  ...
  def build_streak
    if object.score > 0
      zero_date = object.day.user.days.joins(:metric_scores).where('days.date <= ? AND metric_scores.score = 0 AND metric_scores.metric_id = ?', object.day.date, object.metric.id).order('days.date desc').limit(1).first.date
